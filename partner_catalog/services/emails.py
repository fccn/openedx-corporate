...
class InvitationEmailService:
    ...
    def send_invitation_accepted_invitee(self, invitation_id: int) -> None:
        ...
        try:
            ...
            logger.warning("Invitation id=%s not found, skipping acceptance invitee email", invitation_id)
            return
        ...
        subject = render_to_string("partner_catalog/emails/invitation_accepted_invitee_subject.txt", context).strip()
        text_body = render_to_string("partner_catalog/emails/invitation_accepted_invitee_body.txt", context)
        html_body = render_to_string("partner_catalog/emails/invitation_accepted_invitee_body.html", context)
        ...
        try:
            msg.send()
            logger.info("Sent acceptance invitee email id=%s to %s", invitation_id, invite_email)
        except Exception:  # pragma: no cover - external mail failures
            logger.exception("Failed sending acceptance invitee email id=%s to %s", invitation_id, invite_email)
            raise
    ...
    def send_invitation_accepted_manager(self, invitation_id: int) -> None:
        ...
        try:
            ...
            logger.warning("Invitation id=%s not found, skipping acceptance manager email", invitation_id)
            return

        manager = invitation.invited_by
        if not manager or not getattr(manager, "email", None):
            logger.warning("Invitation id=%s has no invited_by with email, skipping manager email", invitation_id)
            return
        ...
        subject = render_to_string("partner_catalog/emails/invitation_accepted_manager_subject.txt", context).strip()
        text_body = render_to_string("partner_catalog/emails/invitation_accepted_manager_body.txt", context)
        html_body = render_to_string("partner_catalog/emails/invitation_accepted_manager_body.html", context)
