...
class CatalogLearnerInvitationService:
    ...
    def resend_invitation(self, invitation_id: int, user):
        ...
        emit_catalog_invitation_tracking_event(
            event_name=EVENT_NAME_INVITATION_RESENT,
            invitation=invitation,
    ...
    def _is_invitation_owner(self, user, invitation) -> bool:
        """Return True if the user is the invitee of this invitation."""
        return user == invitation.user or getattr(user, "email", None) == invitation.invite_email

    def _can_manage_invitation(self, user, invitation) -> bool:
        """
        Return True if the user may act on this invitation on the catalog's behalf.

        This is the access rule for manager-initiated actions (cancel, remove,
        resend): the invitee themselves, platform staff/superusers, or an active
        CatalogManager of the invitation's catalog.
        """
        if not getattr(user, "is_authenticated", False):
            return False
        if self._is_invitation_owner(user, invitation) or user.is_staff or user.is_superuser:
            return True
    ...
    def _validate_invitation_status_access(self, user, invitation, new_status: Status) -> bool:
        ...
        if new_status in [Status.ACCEPTED, Status.DECLINED]:
            return getattr(user, "is_authenticated", False) and self._is_invitation_owner(user, invitation)
        if new_status in [Status.REMOVED, Status.CANCELLED]:
            return self._can_manage_invitation(user, invitation)
        return False
