...
def test_ensure_enrollment_logs_when_open_mode_resolution_fails(mocker, orm):
    """
    A failure while resolving the open mode must go through this function's own
    handler, like every other failure during the sync.

    available_modes() hits the DB (CourseMode.modes_for_course), so it can raise
    just like get_enrollment/add_enrollment. When the resolution sat outside the
    try block the exception escaped unlogged, leaving no trace of the very kind
    of unhandled failure this service is meant to report.
    """
    _ = orm
    ...
    log = mocker.patch.object(pe.logger, "exception")

    with pytest.raises(RuntimeError):
        pe.ensure_edx_platform_enrollment(user_id=1, catalog_course_id=999, target_mode="audit")

    log.assert_called_once()
    assert "Failed to ensure enrollment" in log.call_args.args[0]
